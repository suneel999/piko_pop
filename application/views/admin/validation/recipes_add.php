<script>
    $(document).ready(function() {
        ...
        $('#youtube_url').on('input change', function() {
            ...
            if (id) {
                $('#youtube_thumb_img').attr('src', 'https://img.youtube.com/vi/' + id + '/hqdefault.jpg');
                $('#youtube_thumb_preview').show();
            } else {
                $('#youtube_thumb_preview').hide();
            }
        });
        ...
        $('#add_recipe').on('submit', function(e) {
            ...
            $.ajax({
                url: base_url + 'admin_root/recipes/save_recipe',
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                success: function(result) {
                    ...
                    if (obj.status == 200) {
                        ...
                        $('#save_btn').prop('disabled', false).html('<i class="mdi mdi-content-save me-1"></i> Save Recipe');
                    }
                },
                error: function() {
